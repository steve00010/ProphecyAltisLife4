...
if(pbh_pbh_life_cash >= _cost) then
{
	...
	pbh_pbh_life_cash = pbh_pbh_life_cash - _cost;
	...
	{
		pbh_pbh_life_cash = pbh_pbh_life_cash + _winAmount;
...
{
	if(pbh_pbh_life_cash < _cost) exitWith {hint (format["You need $%1 to play this machine.",_cost]); 
	//5 cutText ["","PLAIN"]; 
	life_is_processing = false;};
};
